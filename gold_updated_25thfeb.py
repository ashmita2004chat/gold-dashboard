...
DEMAND_FILE = "Gold Demand -Tables-Q2-2025-EN.xlsx"

GOLDSMITH_FILE = "Goldsmith.xlsx"
GOLDSMITH_CODES = {"711420"}
HS6_PAGES = [
    ...
    ("711420 — Articles of Gold Smith", "711420"),
]
...
def _kpi_card(icon: str, label: str, value: str, sub: str = "") -> None:
    ...
    # IMPORTANT: keep HTML left-aligned (no leading indentation), otherwise
    # Streamlit/Markdown may render it as a code block.
    html = dedent(
        f"""
        <div class="kpi">
...
# Chart label helpers
# =========================
def _add_line_point_labels(fig, fmt: str = "{:,.2f}", **_ignored):
...
def show_gold_prices() -> None:
    ...
    with tabs[1]:
        ...
        if available_n == 0:
            ...
            # Plot (exactly Top N)
            plot_df = top_df.sort_values("price", ascending=True).copy()
            fig2 = px.bar(
                plot_df,
                x="price",
                y="label",
...
# =========================
# Gold demand tables (World Gold Council) — workbook integration
# =========================
@st.cache_data(show_spinner=False)
def _wgc_read_sheet(path: str, sheet_name: str) -> pd.DataFrame:
    return pd.read_excel(path, sheet_name=sheet_name, header=None)

def _wgc_trim(df: pd.DataFrame) -> pd.DataFrame:
    """Drop fully-empty rows/cols (keeps internal blanks)."""
    return df.dropna(axis=0, how="all").dropna(axis=1, how="all")

def _wgc_is_year(v) -> bool:
    try:
        iv = int(float(v))
        return 1900 <= iv <= 2100
    except Exception:
...
def _wgc_is_quarter_label(v) -> bool:
    if not isinstance(v, str):
        return False
    return bool(re.fullmatch(r"\s*Q[1-4]'\d{2}\s*", v.strip()))

def _wgc_split_annual_quarterly(df_raw: pd.DataFrame, label_name: str = "Category") -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Many WGC tables have a wide layout:
      - first block: annual years (2010…2024)
      - then: YoY columns
      - then: quarterly periods (Q1'10…Q2'25)
    We extract two matrix views: annual and quarterly.
    """
    df = _wgc_trim(df_raw).copy().reset_index(drop=True)
    if df.empty:
        return pd.DataFrame(), pd.DataFrame()

    # Find header row (contains many years and/or quarterly labels)
    header_row = None
    for i in range(min(len(df), 120)):
        row = df.iloc[i].tolist()
        years = [v for v in row if _wgc_is_year(v)]
        qs = [v for v in row if _wgc_is_quarter_label(v)]
        if len(set(map(int, years))) >= 8 or len(qs) >= 8:
            header_row = i
            break
    if header_row is None:
        # Fallback: return trimmed (single view)
        out = df.copy()
    ...
    # Label column = first column (works for this workbook structure)
    label_col = cols[0]

    # Annual + quarterly columns from header row
    annual_cols = [c for c in cols if _wgc_is_year(df.at[header_row, c])]
    quarter_cols = [c for c in cols if _wgc_is_quarter_label(df.at[header_row, c])]

    # Build annual matrix
    annual = pd.DataFrame()
    if annual_cols:
        annual = df.loc[header_row + 1 :, [label_col] + annual_cols].copy()
        annual = annual.rename(columns={label_col: label_name})
        annual = annual[annual[label_name].notna()].copy()
        annual[label_name] = annual[label_name].astype(str).str.strip()
        annual = annual[~annual[label_name].str.contains(r"^Source:", case=False, na=False)]
        annual = annual[annual[label_name].str.lower().ne("nan")]

        # Clean numbers (keep '-' as NaN)
        for c in annual_cols:
            annual[c] = pd.to_numeric(annual[c].replace("-", np.nan), errors="coerce")

        # Rename year columns to int years (stable)
        annual = annual.rename(columns={c: int(float(df.at[header_row, c])) for c in annual_cols})

        # Drop rows that are purely headers/titles
        drop_like = ["gold", "demand", "presentation", "annually", "quarterly"]
        mask_title = annual[label_name].str.lower().apply(lambda s: any(k in s for k in drop_like))
        # Keep data rows with at least one numeric
        num_cols = [c for c in annual.columns if c != label_name]
        annual = annual[~(mask_title & annual[num_cols].isna().all(axis=1))].copy()
        annual = annual.reset_index(drop=True)

    # Build quarterly matrix
    quarterly = pd.DataFrame()
    if quarter_cols:
        quarterly = df.loc[header_row + 1 :, [label_col] + quarter_cols].copy()
        quarterly = quarterly.rename(columns={label_col: label_name})
        quarterly = quarterly[quarterly[label_name].notna()].copy()
        quarterly[label_name] = quarterly[label_name].astype(str).str.strip()
        quarterly = quarterly[~quarterly[label_name].str.contains(r"^Source:", case=False, na=False)]
        quarterly = quarterly[quarterly[label_name].str.lower().ne("nan")]

        # Rename quarters to cleaned labels
        qnames = {c: str(df.at[header_row, c]).strip() for c in quarter_cols}
        quarterly = quarterly.rename(columns=qnames)

        for c in qnames.values():
            quarterly[c] = pd.to_numeric(quarterly[c].replace("-", np.nan), errors="coerce")

        # Drop header-ish rows with no numeric content
        qcols = [c for c in quarterly.columns if c != label_name]
        quarterly = quarterly[~(quarterly[qcols].isna().all(axis=1) & quarterly[label_name].str.contains("tonnes|taxonomy|year-on-year", case=False, na=False))].copy()
        quarterly = quarterly.reset_index(drop=True)

    return annual, quarterly

def _wgc_parse_two_block_highlights(df_raw: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Exec Summary layout: two side-by-side blocks (annual + Q2 snapshot).
    Returns: annual_highlights, q2_highlights.
    """
    df = _wgc_trim(df_raw).copy().reset_index(drop=True)
    if df.empty:
        return pd.DataFrame(), pd.DataFrame()

    # Expected columns after trimming (workbook is stable): left uses cols 1,2,3,5 ; right uses cols 7,8,9,11
    cols = list(df.columns)
    left_cols = [c for c in cols if c in [1, 2, 3, 5]]
    right_cols = [c for c in cols if c in [7, 8, 9, 11]]

    annual = pd.DataFrame()
    if len(left_cols) >= 4:
        tmp = df[left_cols].copy()
        tmp.columns = ["Metric", "2023", "2024", "YoY_%"]
        tmp = tmp[tmp["Metric"].notna()].copy()
        tmp["Metric"] = tmp["Metric"].astype(str).str.strip()
        tmp = tmp[~tmp["Metric"].str.contains("Data Highlights|Tonnes|Source", case=False, na=False)]
        tmp["2023"] = pd.to_numeric(tmp["2023"].replace("-", np.nan), errors="coerce")
        tmp["2024"] = pd.to_numeric(tmp["2024"].replace("-", np.nan), errors="coerce")
        tmp["YoY_%"] = pd.to_numeric(tmp["YoY_%"].replace("-", np.nan), errors="coerce")
        annual = tmp.reset_index(drop=True)

    q2 = pd.DataFrame()
    if len(right_cols) >= 4:
        tmp = df[right_cols].copy()
        tmp.columns = ["Metric", "Q2_24", "Q2_25", "YoY_%"]
        tmp = tmp[tmp["Metric"].notna()].copy()
        tmp["Metric"] = tmp["Metric"].astype(str).str.strip()
        tmp = tmp[~tmp["Metric"].str.contains("Data Highlights|Tonnes|Source", case=False, na=False)]
        tmp["Q2_24"] = pd.to_numeric(tmp["Q2_24"].replace("-", np.nan), errors="coerce")
        tmp["Q2_25"] = pd.to_numeric(tmp["Q2_25"].replace("-", np.nan), errors="coerce")
        tmp["YoY_%"] = pd.to_numeric(tmp["YoY_%"].replace("-", np.nan), errors="coerce")
        q2 = tmp.reset_index(drop=True)

    return annual, q2

def _wgc_parse_snapshot(df_raw: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Snapshot layout: left block (Jewellery/Investment/Central banks) + right block (Technology/Supply).
    Returns: left_table, right_table with a 'Section' column.
    """
    df = _wgc_trim(df_raw).copy().reset_index(drop=True)
    if df.empty:
        return pd.DataFrame(), pd.DataFrame()
    ...
    def _block(item_c, v1_c, v2_c, yoy_c):
        if not all(c in cols for c in [item_c, v1_c, v2_c, yoy_c]):
            return pd.DataFrame()

        tmp = df[[item_c, v1_c, v2_c, yoy_c]].copy()
        tmp.columns = ["_item", "_v1", "_v2", "_yoy"]

        section = None
        out = []
        for _, r in tmp.iterrows():
            item = r["_item"]
            v1, v2, yoy = r["_v1"], r["_v2"], r["_yoy"]

            if pd.notna(item) and (pd.isna(v1) and pd.isna(v2)) and isinstance(item, str):
                # Section header
                s = item.strip()
                if s and not re.search(r"tonnes|year-on-year", s, re.I):
                    section = s
                continue
            ...
            if not item_s or re.search(r"tonnes|year-on-year", item_s, re.I):
                continue
            ...
            out.append(
                {
                    "Section": section or "",
                    "Item": item_s,
                    "Q2'24": pd.to_numeric(v1 if v1 != "-" else np.nan, errors="coerce"),
                    "Q2'25": pd.to_numeric(v2 if v2 != "-" else np.nan, errors="coerce"),
                    "YoY_%": pd.to_numeric(yoy if yoy != "-" else np.nan, errors="coerce"),
                }
            )
        return pd.DataFrame(out)

    left = _block(1, 2, 3, 5)
    right = _block(7, 8, 9, 11)
    return left, right


def _wgc_trend_symbol(v) -> str:
    try:
        if pd.isna(v):
            return ""
        v = float(v)
...
def _wgc_color_for_trend(sym: str) -> str:
    if sym == "▲":
...
def _wgc_style_table(df: pd.DataFrame, trend_col: str | None = None, yoy_col: str | None = None, decimals: int = 2):
    """Return a Pandas Styler (with safe fallbacks if styling isn't supported by the runtime)."""
    ...
    for c in df.columns:
        if pd.api.types.is_numeric_dtype(df[c]):
            if yoy_col is not None and c == yoy_col:
                fmt[c] = "{:+." + str(decimals) + "f}"
            else:
                fmt[c] = "{:,.2f}" if decimals == 2 else "{:,.1f}"
    if fmt:
        styler = styler.format(fmt)

    # Color trend symbol and YoY values
    if trend_col is not None and trend_col in df.columns:
        styler = styler.map(lambda x: _wgc_color_for_trend(x), subset=[trend_col])

    if yoy_col is not None and yoy_col in df.columns and pd.api.types.is_numeric_dtype(df[yoy_col]):
        def _yoy_style(v):
            try:
                if pd.isna(v):
                    return ""
            ...
            if v < 0:
                return "color:#dc2626; font-weight:800;"
            return "color:#64748b; font-weight:800;"
        styler = styler.map(_yoy_style, subset=[yoy_col])

    return styler


def _wgc_sorted_periods(periods: list[str]) -> list[str]:
    """Sort years or quarter labels (e.g., Q1'10) in chronological order."""
    def _key(p: str):
        p = str(p).strip()
        if re.fullmatch(r"\d{4}", p):
            return (0, int(p), 0)
        m = re.fullmatch(r"Q([1-4])\'(\d{2})", p)
        if m:
            q = int(m.group(1))
            yy = int(m.group(2))
...
def _wgc_plot_series(df: pd.DataFrame, label_col: str, key_prefix: str):
    """Small helper to plot a selected row as a line chart."""
    ...
    options = df[label_col].dropna().astype(str).tolist()
    if not options:
        return

    sel = st.selectbox("Select series to visualise", options, index=0, key=f"{key_prefix}_sel")
    row = df[df[label_col].astype(str) == sel]
    if row.empty:
        return
    row = row.iloc[0]

    periods = [str(c) for c in num_cols]
    periods_sorted = _wgc_sorted_periods(periods)

    values = []
    for p in periods_sorted:
        c = int(p) if p.isdigit() and int(p) in df.columns else p
        # columns might be int years; handle both
        if c in df.columns:
            values.append(row[c])
        else:
            # try int version
            try:
                ci = int(p)
                values.append(row[ci] if ci in df.columns else np.nan)
            except Exception:
                values.append(np.nan)

    ts = pd.DataFrame({"Period": periods_sorted, "Value": values})
    fig = px.line(ts, x="Period", y="Value", markers=True, color_discrete_sequence=["#B8860B"])
    # Theme: match dashboard (gold line) + optional data labels
    fig.update_traces(
        line=dict(color="#B8860B", width=3),
        marker=dict(color="#B8860B", size=7),
    )
    fig.update_layout(
        template="plotly_white",
        height=320,
...
def show_gold_demand_wgc() -> None:
    demand_path = resolve_file(DEMAND_FILE)
    if not demand_path.exists():
        st.error(f"Missing {DEMAND_FILE}. Place it in the working folder, /mnt/data, or /content.")
        return
        ...
        <div class="header-wrap hero">
          <div class="hero-row">
            <div class="hero-left">
              <div class="hero-icon">{GOLD_ICON_SVG}</div>
              <div>
                <div class="h1">Gold Demand Trends</div>
                <div class="hsub">World Gold Council tables • Annual + Quarterly (calendar-year / calendar-quarter) • File: <b>{demand_path.name}</b></div>
              </div>
            </div>
            <div class="hero-right">
              <div class="badge">Latest: Q2’25</div>
            </div>
          </div>
          <div class="hero-art">{GOLD_ART_SVG}</div>
        </div>
    ...
    def _render_matrix_sheet(sheet_name: str, label_name: str) -> None:
        """Render WGC wide matrix sheets (annual + quarterly) in a clean, interactive way."""
        try:
            df_raw = _wgc_read_sheet(str(demand_path), sheet_name)
            if df_raw is None or df_raw.empty:
                st.info("No data found in this sheet.")
                return

            annual, quarterly = _wgc_split_annual_quarterly(df_raw, label_name=label_name)

            view = st.radio(
                "View",
                ["Annual", "Quarterly"],
                horizontal=True,
                key=f"wgc_{sheet_name}_view",
            )

            df_view = annual if view == "Annual" else quarterly
            if df_view is None or df_view.empty:
                st.info("This sheet does not contain a readable matrix for the selected view.")
                return

            # Identify period columns
            period_cols = [c for c in df_view.columns if c != label_name]

            # Compute a simple YoY % for the latest period (Annual: latest vs previous year; Quarterly: same quarter prev year if available)
            ...
            if view == "Annual":
                years = [c for c in period_cols if isinstance(c, int)]
                years = sorted(years)
                if len(years) >= 2:
                    latest_col, prev_col = years[-1], years[-2]
            else:
                quarters = [c for c in period_cols if isinstance(c, str) and _wgc_is_quarter_label(c)]
                quarters = _wgc_sorted_periods(quarters)
                if quarters:
                    latest_col = quarters[-1]
                    # try same quarter previous year
                    m = re.fullmatch(r"\s*Q([1-4])'(\d{2})\s*", str(latest_col).replace("’", "'").strip())
                    if m:
                        q = m.group(1)
                        yy = int(m.group(2))
                        prev_label = f"Q{q}'{yy-1:02d}"
                        if prev_label in df_show.columns:
                            prev_col = prev_label
                    # fallback: previous period in sorted list
                    if prev_col is None and len(quarters) >= 2:
                        prev_col = quarters[-2]

            if latest_col is not None and prev_col is not None and latest_col in df_show.columns and prev_col in df_show.columns:
                prev_vals = df_show[prev_col].astype(float)
                latest_vals = df_show[latest_col].astype(float)
                yoy = (latest_vals - prev_vals) / prev_vals.replace(0, np.nan) * 100.0
                df_show["YoY_%"] = yoy
                df_show["Trend"] = df_show["YoY_%"].apply(_wgc_trend_symbol)

                # Place Trend/YoY at the end
                base_cols = [c for c in df_view.columns if c != label_name]
                df_show = df_show[[label_name] + base_cols + ["Trend", "YoY_%"]]

            # Controls
            c1, c2 = st.columns([1.1, 1.9])
            with c1:
                topn = st.slider("Top N (latest period)", 5, 30, 10, key=f"wgc_{sheet_name}_topn")
            with c2:
                if latest_col is not None:
                    st.markdown(
                        f"<div class='small-note'><b>Latest period:</b> {latest_col}"
                        + (f" &nbsp;•&nbsp; YoY vs <b>{prev_col}</b>" if prev_col is not None else "")
                        + "</div>",
                        unsafe_allow_html=True,
                    )

            # Chart: Top N by latest period
            if latest_col is not None and latest_col in df_view.columns:
                plot_df = df_view[[label_name, latest_col]].copy()
                plot_df = plot_df[plot_df[label_name].notna()].copy()
                plot_df[label_name] = plot_df[label_name].astype(str).str.strip()
                plot_df = plot_df[~plot_df[label_name].str.contains(r"^Source:", case=False, na=False)]
                plot_df[latest_col] = pd.to_numeric(plot_df[latest_col], errors="coerce")
                plot_df = plot_df.sort_values(latest_col, ascending=False).head(topn)
                plot_df = plot_df.sort_values(latest_col, ascending=True)

                fig = px.bar(plot_df, x=latest_col, y=label_name, orientation="h")
                fig.update_traces(marker_color="#B8860B")
                fig.update_layout(template="plotly_white", height=420, margin=dict(l=10, r=10, t=10, b=10))
                if show_labels:
                    _add_bar_labels(fig, orientation="h", fmt="{:,.2f}")
                st.plotly_chart(fig, use_container_width=True)
            ...
            styler = None
            try:
                if "YoY_%" in df_show.columns:
                    styler = _wgc_style_table(df_show, trend_col="Trend" if "Trend" in df_show.columns else None, yoy_col="YoY_%", decimals=2)
            except Exception:
                styler = None
            ...
            st.download_button(
                ...
                file_name=f"wgc_{sheet_name.lower().replace(' ', '_')}.csv",
                mime="text/csv",
                key=f"wgc_{sheet_name}_dl",
            )

            # Series explorer
            st.markdown("##### Series explorer")
            _wgc_plot_series(df_view, label_col=label_name, key_prefix=f"wgc_{sheet_name}_{view.lower()}")

        except Exception as e:
            st.error(f"Could not render '{sheet_name}'.")
            st.exception(e)


    # ---- Exec Summary ----
    with sheet_tabs[0]:
        df_raw = _wgc_read_sheet(str(demand_path), "Exec Summary")
        annual, q2 = _wgc_parse_two_block_highlights(df_raw)

        def _render_highlight_block(df_h: pd.DataFrame, label: str, v_prev: str, v_curr: str):
            st.markdown(f"### {label} (Tonnes)")
            if df_h.empty:
                st.info("No data found.")
                return

            # Add trend column like the Excel icons
            df_show = df_h.copy()
            df_show["Trend"] = df_show["YoY_%"].apply(_wgc_trend_symbol)
            ...
            icons = {
                ...
                "Total Bar and Coin": "🪙",
                "ETFs and Similar Products": "📈",
                "Central Bank and Other Institutions": "🏛️",
            }
            ...
            def _kpi_card_html(icon: str, label: str, value: str, sub: str = "") -> str:
                ...
                if digits >= 13 or len(v) >= 16:
                    ...
                    val_style = "font-size:22px; line-height:1.05; word-break:break-word;"
                # IMPORTANT: no leading indentation; otherwise Markdown treats it as code.
                return dedent(
                    f"""
                    <div class="kpi">
                      <div class="ico">{icon}</div>
            ...
            for m in metric_order:
                r = df_show[df_show["Metric"].astype(str).str.strip().str.lower() == m.lower()]
                if r.empty:
                    continue
                r = r.iloc[0]
                val = r.get(v_curr, np.nan)
                yoy = r.get("YoY_%", np.nan)
                ...
                sub = "YoY: —"
                if not pd.isna(yoy):
                    sym = _wgc_trend_symbol(yoy)
                    if yoy > 0:
                        sub = f"<span style='color:#16a34a;font-weight:800;'>{sym} {yoy:.2f}% YoY</span>"
                    elif yoy < 0:
                        sub = f"<span style='color:#dc2626;font-weight:800;'>{sym} {yoy:.2f}% YoY</span>"
                    else:
                        sub = f"<span style='color:#64748b;font-weight:800;'>{sym} {yoy:.2f}% YoY</span>"

                cards.append(_kpi_card_html(icons.get(m, "🟡"), m, val_str, sub))

            if cards:
                st.markdown('<div class="kpi-grid">' + "".join(cards) + "</div>", unsafe_allow_html=True)

            st.markdown("##### Full table")
            # Reorder columns to match a clean read
            ordered = ["Metric", v_prev, v_curr, "Trend", "YoY_%"]
            df_show = df_show[[c for c in ordered if c in df_show.columns]].copy()

            styler = _wgc_style_table(df_show, trend_col="Trend", yoy_col="YoY_%", decimals=2)
            ...
            st.download_button(
                f"Download {label.lower()} highlights (CSV)",
                data=df_show.to_csv(index=False).encode("utf-8"),
                file_name=f"wgc_exec_summary_{label.lower().replace(' ', '_')}.csv",
                mime="text/csv",
            )

        c1, c2 = st.columns(2)
        with c1:
            _render_highlight_block(annual, "Annual highlights", "2023", "2024")
        with c2:
            _render_highlight_block(q2, "Q2 highlights", "Q2_24", "Q2_25")
    # ---- Snapshot ----
    with sheet_tabs[1]:
        df_raw = _wgc_read_sheet(str(demand_path), "Snapshot")
        left, right = _wgc_parse_snapshot(df_raw)

        c1, c2 = st.columns(2)

        with c1:
            st.markdown("### Snapshot (Left block)")
            if left.empty:
                st.info("No data found.")
            else:
                left_show = left.copy()
                left_show["Trend"] = left_show["YoY_%"].apply(_wgc_trend_symbol)
                left_show = left_show[["Section", "Item", "Q2'24", "Q2'25", "Trend", "YoY_%"]]
                styler = _wgc_style_table(left_show, trend_col="Trend", yoy_col="YoY_%", decimals=2)
                try:
                    st.dataframe(styler, use_container_width=True, height=520, hide_index=True)
                except Exception:
                    st.dataframe(left_show, use_container_width=True, height=520, hide_index=True)

                st.download_button(
                    "Download snapshot (left) CSV",
                    data=left_show.to_csv(index=False).encode("utf-8"),
                    file_name="wgc_snapshot_left.csv",
                    mime="text/csv",
                )

        with c2:
            st.markdown("### Snapshot (Right block)")
            if right.empty:
                st.info("No data found.")
            else:
                right_show = right.copy()
                right_show["Trend"] = right_show["YoY_%"].apply(_wgc_trend_symbol)
                right_show = right_show[["Section", "Item", "Q2'24", "Q2'25", "Trend", "YoY_%"]]
                styler = _wgc_style_table(right_show, trend_col="Trend", yoy_col="YoY_%", decimals=2)
                try:
                    st.dataframe(styler, use_container_width=True, height=520, hide_index=True)
                except Exception:
                    st.dataframe(right_show, use_container_width=True, height=520, hide_index=True)

                st.download_button(
                    "Download snapshot (right) CSV",
                    data=right_show.to_csv(index=False).encode("utf-8"),
                    file_name="wgc_snapshot_right.csv",
                    mime="text/csv",
                )
    with sheet_tabs[2]:
        _render_matrix_sheet("Gold Balance", "Category")
    with sheet_tabs[3]:
        _render_matrix_sheet("Jewellery", "Country/Region")
    with sheet_tabs[4]:
        _render_matrix_sheet("Bar and Coin", "Country/Region")
    with sheet_tabs[5]:
        _render_matrix_sheet("Consumer", "Country/Region")
    with sheet_tabs[6]:
        _render_matrix_sheet("Consumer per Capita", "Country/Region")
    with sheet_tabs[7]:
        _render_matrix_sheet("Gold Prices", "Taxonomy")
    with sheet_tabs[8]:
        _render_matrix_sheet("India Supply", "Item")

    # ---- ETFs ----
    with sheet_tabs[9]:
        df_raw = _wgc_read_sheet(str(demand_path), "ETFs")
        df = _wgc_trim(df_raw).copy().reset_index(drop=True)

        # -----------------------------
        # Table 1: Top 10 physically-backed ETFs
        # -----------------------------
        fund_row = None
        for i in range(min(len(df), 120)):
            if (df.iloc[i].astype(str).str.strip() == "Fund").any():
                fund_row = i
                break

        top10 = pd.DataFrame()
        if fund_row is not None:
            cols_needed = [c for c in [1, 2, 3, 4, 5] if c in df.columns]
            block = df.loc[fund_row:, cols_needed].copy()
            # Standardise
            col_map = {1: "Fund", 2: "Country", 3: "Holdings_Tonnes", 4: "Trend", 5: "YoY_%"}
            block.columns = [col_map.get(c, str(c)) for c in cols_needed]
            block = block[block["Fund"].notna()].copy()
            block["Fund"] = block["Fund"].astype(str).str.strip()
            block = block[~block["Fund"].str.contains(r"^Source:", case=False, na=False)]
            block = block[~block["Fund"].str.contains(r"Top 10 physically-backed", case=False, na=False)]
            block = block[block["Fund"].str.lower().ne("fund")]
            block = block[block["Fund"].str.lower().ne("nan")]

            if "Holdings_Tonnes" in block.columns:
                block["Holdings_Tonnes"] = pd.to_numeric(block["Holdings_Tonnes"].replace("-", np.nan), errors="coerce")
            if "YoY_%" in block.columns:
                block["YoY_%"] = pd.to_numeric(block["YoY_%"].replace("-", np.nan), errors="coerce")

            if "Trend" not in block.columns:
                block["Trend"] = block["YoY_%"].apply(_wgc_trend_symbol)
            else:
                # Fill missing trend using YoY
                block["Trend"] = block["Trend"].fillna("").astype(str).str.strip()
                block.loc[block["Trend"] == "", "Trend"] = block.loc[block["Trend"] == "", "YoY_%"].apply(_wgc_trend_symbol)

            top10 = block.reset_index(drop=True)
            if "Holdings_Tonnes" in top10.columns:
                top10 = top10.sort_values("Holdings_Tonnes", ascending=False, na_position="last").reset_index(drop=True)

        # -----------------------------
        # Table 2: AuM by region (quarterly)
        # -----------------------------
        tax_row = None
        for i in range(min(len(df), 200)):
            if (df.iloc[i].astype(str).str.strip() == "Taxonomy").any():
                tax_row = i
                break
        ...
        if tax_row is not None:
            ...
            # Identify quarter columns on that row (robust to index/label issues)
            for j, c in enumerate(df.columns):
                v = df.iat[tax_row, j]
                if isinstance(v, str) and _wgc_is_quarter_label(v.strip()):
                    q_cols.append(c)
                    q_names[c] = v.strip()

            yoy_tonnage_col = None
            yoy_pct_col = None
            for j, c in enumerate(df.columns):
                v = df.iat[tax_row, j]
                if isinstance(v, str):
                    vv = v.lower()
                    if "year-on-year" in vv and "tonnage" in vv:
                        yoy_tonnage_col = c
                    if "year-on-year" in vv and "%" in vv:
                        yoy_pct_col = c

            cols_keep = [1] + q_cols
            if yoy_tonnage_col is not None:
                cols_keep.append(yoy_tonnage_col)
            if yoy_pct_col is not None:
                cols_keep.append(yoy_pct_col)

            if all(c in df.columns for c in cols_keep):
                block = df.loc[tax_row + 1 :, cols_keep].copy()
                # stop at Source row if present
                if 1 in block.columns:
                    src_idx = block[1].astype(str).str.contains(r"^Source:", case=False, na=False)
                    if src_idx.any():
                        block = block.loc[: src_idx.idxmax() - 1, :]

                col_names = ["Region"] + [q_names[c] for c in q_cols]
                if yoy_tonnage_col is not None:
                    col_names.append("YoY_tonnage_change")
                if yoy_pct_col is not None:
                    col_names.append("YoY_%")

                block.columns = col_names

                block = block[block["Region"].notna()].copy()
                block["Region"] = block["Region"].astype(str).str.strip()
                block = block[~block["Region"].str.contains(r"^Source:", case=False, na=False)]
                block = block[block["Region"].str.lower().ne("taxonomy")]
                block = block[block["Region"].str.lower().ne("nan")]

                # Numeric conversion
                for c in block.columns:
                    if c == "Region":
                        continue
                    block[c] = pd.to_numeric(block[c].replace("-", np.nan), errors="coerce")

                # Add trend symbol from YoY_%
                if "YoY_%" in block.columns:
                    block["Trend"] = block["YoY_%"].apply(_wgc_trend_symbol)
                    # place Trend before YoY_%
                    cols_order = [c for c in block.columns if c not in ["Trend", "YoY_%"]] + ["Trend", "YoY_%"]
                    block = block[cols_order]

                region = block.reset_index(drop=True)

        st.markdown("### ETFs overview")
        st.markdown("<div class='small-note'>Physically-backed gold ETFs • AuM/holdings are shown in tonnes • Icons and colours follow the Excel-style up/down movement.</div>", unsafe_allow_html=True)

        left_col, right_col = st.columns(2)

        with left_col:
            st.markdown("#### Top 10 physically-backed gold ETFs")
            if top10.empty:
                st.info("No Top-10 ETF table detected in the sheet.")
            else:
                styler = _wgc_style_table(top10, trend_col="Trend", yoy_col="YoY_%", decimals=2)
                try:
                    st.dataframe(styler, use_container_width=True, height=420, hide_index=True)
                except Exception:
                    st.dataframe(top10, use_container_width=True, height=420, hide_index=True)

                # Bar chart
                if "Fund" in top10.columns and "Holdings_Tonnes" in top10.columns:
                    fig = px.bar(top10, x="Holdings_Tonnes", y="Fund", orientation="h")
                    fig.update_traces(marker_color="#B8860B")
                ...
                st.download_button(
                    "Download Top-10 ETFs (CSV)",
                    data=top10.to_csv(index=False).encode("utf-8"),
                    file_name="wgc_etfs_top10.csv",
                    mime="text/csv",
                )

        with right_col:
            st.markdown("#### AuM by region (quarterly)")
            if region.empty:
                st.info("No region table detected in the sheet.")
            else:
                # Show line chart for a selected region
                q_cols = [c for c in region.columns if isinstance(c, str) and _wgc_is_quarter_label(c)]
                if q_cols:
                    sel_region = st.selectbox("Choose region", region["Region"].tolist(), key="wgc_region_sel")
                    rrow = region[region["Region"] == sel_region].iloc[0]
                    periods = _wgc_sorted_periods(q_cols)
                    ts = pd.DataFrame({"Quarter": periods, "Tonnes": [rrow[p] for p in periods]})
                    fig = px.line(ts, x="Quarter", y="Tonnes", markers=True, color_discrete_sequence=["#B8860B"], title=None)
                    # Match dashboard theme (gold) + apply data-label toggle (same as other charts)
                    fig.update_traces(
                        line=dict(color="#B8860B", width=3),
                        marker=dict(color="#B8860B", size=7),
                        hovertemplate="%{x}<br>Tonnes: %{y:,.1f}<extra></extra>",
                    )
                    fig.update_layout(
                        template="plotly_white",
                        height=320,
                ...
                styler = _wgc_style_table(region, trend_col="Trend" if "Trend" in region.columns else None, yoy_col="YoY_%" if "YoY_%" in region.columns else None, decimals=2)
                try:
                    st.dataframe(styler, use_container_width=True, height=420, hide_index=True)
                except Exception:
                    st.dataframe(region, use_container_width=True, height=420, hide_index=True)

                st.download_button(
                    "Download region table (CSV)",
                    data=region.to_csv(index=False).encode("utf-8"),
                    file_name="wgc_etfs_region_quarterly.csv",
                    mime="text/csv",
                )
...
_goldsmith_ok = resolve_file(GOLDSMITH_FILE).exists()
nav_items = ["Production", "Gold Prices", "Gold Demand Trends (WGC)"] + [
    p[0]
    for p in HS6_PAGES
    if (
        (p[1] is None)
        or (p[1] in JEWELLERY_CODES and _jew_ok)
        or (p[1] in GOLDSMITH_CODES and _goldsmith_ok)
        or (p[1] not in JEWELLERY_CODES and p[1] not in GOLDSMITH_CODES)
    )
]
...
# Data labels (default ON): keep behavior consistent across all tabs, incl. WGC charts
...
if page == "Production":
    ...
    show_gold_prices()
elif page == "Gold Demand Trends (WGC)":
    show_gold_demand_wgc()
else:
    if page == "Trade — 7108 (Total)":
        ...
        if sheet_code is None:
            ...
            elif sheet_code in GOLDSMITH_CODES:
                trade_path_use = resolve_file(GOLDSMITH_FILE)
            if not trade_path_use.exists():
                ...
                show_trade(("Trade — 711420 (Articles of Gold Smith)" if sheet_code in GOLDSMITH_CODES else f"Trade — {sheet_code}"), imp_w, exp_w, page_key=f"trade_{sheet_code}")
